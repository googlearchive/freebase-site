...
var h = acre.require("helpers_acre");
...
var spec =  [
    ...
    },
    {
        'name' : 'file',
        'id' : 'foo',
        'expected' : make_empty_result({'appid' : '/freebase/site/core' , 'filename' : 'foo'}),
        'options' : { 'file' : true }
    },
    {
...
for (var i in spec) {
    ...
    test(current.name, current, function() {
        var result = h.parse_path(this.id, scope, this.options || null);
