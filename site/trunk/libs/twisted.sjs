...
Deferred.prototype.runCallstack = function(){
    ...
    // callback is the wrong tyoe, so skip
    if ((cb.kind !== "both") &&
...
acre.task.Deferred         = Deferred;
acre.task.DeferredError    = DeferredError;
acre.task.DeferredList     = DeferredList;
acre.task.DeferredDict     = DeferredDict;
acre.task.maybeDeferred    = maybeDeferred;
acre.task.makeDeferred     = makeDeferred;

acre.async.DeferredUrlfetch = makeDeferred(
    acre.async.urlfetch, 
...
freebase_apis.forEach(function(api){
    var sync_name = api["name"];
    
    var deferred_name = "Deferred" + sync_name.split("_").map(function(seg){
         return seg.substr(0,1).toUpperCase() + seg.substr(1);
    }).join("");
    
    acre.freebase[deferred_name] = makeDeferred(
        acre.freebase[sync_name], 
